...
llm_latency_histogram = Histogram(
    'discord_bot_llm_call_duration_seconds',
...
message_processing_latency_histogram = Histogram(
    'discord_bot_message_processing_duration_seconds',
...
cache_latency_histogram = Histogram(
    'discord_bot_cache_lookup_duration_seconds',
...
db_write_latency_histogram = Histogram(
    'discord_bot_db_write_duration_seconds',
...
concurrent_messages = Gauge(
    'discord_bot_concurrent_messages_current',
    'Current concurrent message processing',
    registry=registry
)

peak_concurrent_messages = Gauge(
    'discord_bot_concurrent_messages_peak',
        ...
        try:
            async with session.post(
                ...
                if resp.status == 200:
                    ...
                    self.errors += 1
                    # === Prometheus 上报（LLM 错误）===
                    llm_errors_total.labels(error_type=f'http_{resp.status}').inc()
            ...
            self.errors += 1
            llm_errors_total.labels(error_type='exception').inc()
            llm_calls_total.labels(model=self.config.model, status='error').inc()
            return None
...
class RealLLMLoadTest:
    ...
    async def message_processor(self, message: Dict):
        ...
        concurrent_messages.set(self.concurrent_count)
        peak_concurrent_messages.set(self.peak_concurrent)
        ...
        try:
            ...
            if cache_key in self.cache:
                ...
                # === Prometheus 上报（缓存命中）===
                cache_latency_histogram.observe(cache_lookup_time)
                cache_hits_total.inc()
                messages_processed_total.labels(source='cache_hit').inc()

                await asyncio.sleep(0.005)
                total_time = (time.time() - start_time)
                message_processing_latency_histogram.observe(total_time)

                # 更新缓存命中率
                current_hit_ratio = self.cache_hits / max(1, self.cache_hits + (self.total_messages - self.cache_hits))
                cache_hit_ratio.set(current_hit_ratio)

                logger.info(f"{Colors.OKGREEN}✓ CACHE_HIT{Colors.ENDC} | {msg_id:15s} | {cache_lookup_time*1000:.1f}ms")
            ...
            try:
                ...
                # === Prometheus 上报（LLM 调用成功）===
                llm_calls_total.labels(model=self.llm_config.model, status='success').inc()
                llm_latency_histogram.observe(llm_time)
                ...
                self.db_times.append(db_time_ms)

                # === Prometheus 上报（DB 写入）===
                db_write_latency_histogram.observe(db_time)
                ...
                # === Prometheus 上报（消息处理完成）===
                messages_processed_total.labels(source='llm_call').inc()
                message_processing_latency_histogram.observe(total_time)

                # 更新缓存命中率
                current_hit_ratio = self.cache_hits / max(1, self.total_messages)
                cache_hit_ratio.set(current_hit_ratio)

                # 更新吞吐量
                elapsed = time.time() - self.start_time
                if elapsed > 0:
                    current_throughput = (self.cache_hits + len(self.llm_times)) / elapsed
                    throughput_gauge.set(current_throughput)

                logger.info(f"{Colors.OKGREEN}✓ DONE{Colors.ENDC} | {msg_id:15s} | LLM:{llm_time_ms:7.0f}ms | DB:{db_time_ms:5.1f}ms | Total:{total_time*1000:7.0f}ms")
                self.latencies.append(total_time * 1000)
    ...
    async def run(self):
        ...
        if self.prometheus_pushgateway:
            try:
                ...
                logger.info(f"{Colors.OKGREEN}✓ Prometheus 指标已上报到 {self.prometheus_pushgateway}{Colors.ENDC}")
            except Exception as e:
    ...
    def _print_summary(self, duration):
        ...
        print(f"  discord_bot_message_processing_duration_seconds")
        print(f"  discord_bot_cache_lookup_duration_seconds")
        print(f"  discord_bot_db_write_duration_seconds")
